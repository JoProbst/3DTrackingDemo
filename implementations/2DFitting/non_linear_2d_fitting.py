...
out_folder = "out_non_linear_2D_fitting/"
n_shape_params = 5
...
used_vertices_3 = []
for vert in used_lms_numbers[:observations_used[0]]:
    for i in range(3):
        used_vertices_3.append(3 * vert + i)
rescaled_model = morphablemodel_with_expressions.get_shape_model().get_rescaled_pca_basis()
...
# use chin, nose tip, outer and inner corner of eyes and mouth for transformation
# used_lms = [int(landmark_mapper.convert(lm)) for lm in used_lms]
vertices_obj_space_use = vertices_obj_space[used_lms]
...
mean_zooms = np.zeros(num_people)
...
n_observations_total = np.sum(n_cams_used)
...
for person_idx in range(num_people):
    ...
    params = ba.optimize()
    shape_coeffs[person_idx] = params[1]
    blendshape_coeffs[indices] = params[3]
    trans_matsBA[indices] = params[4]
